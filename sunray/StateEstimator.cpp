...
//  Use dmpUpdateFifo to update the ax, gx, mx, etc. values
    ...
    // CONSOLE.print(",");
    // CONSOLE.println(pitchChange/PI*180.0);
    if ((fabs(scalePI(imuDriver.roll)) > 60.0 / 180.0 * PI) || (fabs(scalePI(imuDriver.pitch)) > 100.0 / 180.0 * PI) || (fabs(rollChange) > 30.0 / 180.0 * PI) || (fabs(pitchChange) > 60.0 / 180.0 * PI))
    {
      dumpImuTilt();
      activeOp->onImuTilt();
...
void computeRobotState()
{
  ...
  if ((gps.solutionAvail) && ((gps.solution == SOL_FIXED) || (gps.solution == SOL_FLOAT)))
  {
    ...
    else if (distGPS > 0.1)
    {
      if ((fabs(motor.linearSpeedSet) > 0) && (fabs(motor.angularSpeedSet) / PI * 180.0 < 45))
      {
        ...
        if (((gps.solution == SOL_FIXED) && (maps.useGPSfixForDeltaEstimation)) || ((gps.solution == SOL_FLOAT) && (maps.useGPSfloatForDeltaEstimation)))
  ...
  if (stateOp == OP_MOW)
    statMowDistanceTraveled += distOdometry / 100.0;
